...
entity ForwardingUnit is
    Port (
       ...
       -- writeRF_WB: in std_logic;
        ...
        exec_rs1, exec_rs2    : in std_logic_vector(4 downto 0);    -- Source register 1 and 2 of the current instructions in ex_stage
...
--forwarding for register rs1.
-- priority to 'prev' (MEM_STAGE) with respect to 'pre_prev(WRITEBACK_STAGE)'
--if exec_rs1 /= "00000" then                                  -- don't forward register x0     
if (writeback_prev = '1' and load_rd_prev = exec_rs1 and load_rd_prev /= "00000") then
    if i_ctr_lui_prev = '1' then                            -- If in stage MEM instruction is a LUI, forward immediate value
            w_ctr_rs1 <= "11";
    else                                                     --else forward from ALU result
            w_ctr_rs1 <= "01";
    end if;
elsif (writeback_pre_prev = '1' and load_rd_pre_prev = exec_rs1 and load_rd_pre_prev /= "00000") then       --forward from WB stage
    w_ctr_rs1 <= "10";
else
    w_ctr_rs1 <= "00";
...
--if exec_rs2 /= "00000" then                                     -- don't forward register x0
    if (writeback_prev = '1' and load_rd_prev = exec_rs2 and load_rd_prev /= "00000") then
        if i_ctr_lui_prev = '1' then                            -- If in stage MEM instruction is a LUI, forward immediate value
            w_ctr_rs2 <= "11";
        else                                                    --else forward from ALU result
            w_ctr_rs2 <= "01"; 
        end if;
    elsif (writeback_pre_prev = '1' and load_rd_pre_prev = exec_rs2 and load_rd_pre_prev /="00000") then       --forward from WB stage
        w_ctr_rs2 <= "10";
    else 
        w_ctr_rs2 <= "00";
    end if;
