...
entity pipe_ID_EX is
 ...
 -- Inputs*******************************************************
        ...
        i_predict_trgt      : in std_logic_vector(31 downto 2);
...
-- Outputs**********************************************************
        ...
        o_predict_trgt      : out std_logic_vector(31 downto 2);
...
architecture rtl of pipe_ID_EX is
    ...
    signal r_predict_taken                                      : std_logic;
    signal r_predict_trgt                                       : std_logic_vector(31 downto 2);
...
--since branch is decied in exe stage <=  flush this pipe only if there is not a stall, so that the jump instruction can propagate 
        elsif rising_edge(CLK) then
            if flusher = '1' and enable = '1' then
                r_instr_valid     <= '0';
                r_RegisterWrite   <= '0';
                r_CTR_branch      <= '0';
                r_MemWrite        <= '0';
                ...
                r_predict_trgt    <= i_predict_trgt;
                r_predict_taken   <= i_predict_taken;
...
-- Outputs 
    o_predict_trgt          <= r_predict_trgt;
    o_predict_taken         <= r_predict_taken;
